...
class StockTakeService:
    ...
    def create_stocktake(warehouse_id, take_type, product_ids, user, remark=None, planned_date=None):
        ...
        try:
            ...
            # 根据盘点类型获取商品
            if take_type in (StockTake.TYPE_FULL, StockTake.TYPE_CYCLE) and not product_ids:
                # 全盘/周期盘点：默认覆盖仓库所有有库存的商品，保证现场扫码流程可执行。
            ...
            for product_id in product_ids:
                ...
                item = StockTakeItem(
                    ...
                    shelf_location=stock.shelf_location if stock else None
                )
                db.session.add(item)
    ...
    def batch_input_count(stocktake_id, counts, user):
        ...
        if stocktake.status != StockTake.STATUS_IN_PROGRESS:
            return 0
        for c in counts:
            item = db.session.get(StockTakeItem, c['item_id'])
            if not item or item.take_id != stocktake_id:
                continue
            item.actual_qty = c['actual_qty']
            item.counted_by = user.id
            item.counted_at = utcnow()
            item.remark = c.get('remark')
            success_count += 1
