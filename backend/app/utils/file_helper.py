...
from app.utils.upload_policy import allowed_upload_extensions, normalized_upload_mime, validate_upload_type

DEDICATED_UPLOAD_FOLDERS = {
    'attachments': ('UPLOAD_FILES_FOLDER', 'files'),
    'files': ('UPLOAD_FILES_FOLDER', 'files'),
    'documents': ('UPLOAD_FILES_FOLDER', 'files'),
    'avatars': ('UPLOAD_AVATARS_FOLDER', 'avatars'),
    'avatar': ('UPLOAD_AVATARS_FOLDER', 'avatars'),
    'library': ('UPLOAD_LIBRARY_FOLDER', 'library'),
}


def resolve_upload_destination(folder='attachments'):
    """Resolve a logical upload folder to a configured persistent directory."""
    key = str(folder or 'attachments').strip().lower().replace('\\', '/').split('/')[-1]
    config_key, prefix = DEDICATED_UPLOAD_FOLDERS.get(key, ('UPLOAD_FILES_FOLDER', 'files'))
    root = current_app.config.get(config_key) or os.path.join(current_app.config['UPLOAD_FOLDER'], prefix)
    return root, prefix
...
def save_file(file, folder='attachments'):
    ...
    try:
        ...
        if not validate_upload_type(file, ext):
            current_app.logger.warning(f'save_file: 文件内容类型或签名不允许, ext={ext}, mimetype={file.mimetype}')
            return None
        ...
        if is_cloud_storage_enabled():
            ...
            if result:
                ...
                return safe_name, file_url, file_size, mimetype
            else:
                current_app.logger.warning('save_file: 云存储上传失败，切换到本地存储')
        
        # 本地存储模式
        unique_name = f"{uuid.uuid4().hex}.{ext}"
        
        # 确保上传目录存在
        upload_folder, logical_prefix = resolve_upload_destination(folder)
        ...
        logical_path = f'{logical_prefix}/{unique_name}'
        return display_name, logical_path, file_size, mimetype
